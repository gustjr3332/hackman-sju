...
- **모바일 대응: 반응형 웹으로 결정 (네이티브 앱 안 감).** 목업(`design/mobile-ui/`)은
  ...
  `.auth-name`에 96px 말줄임표(ellipsis) 추가해 해결 (2026-09-23). 로컬에 Supabase가
  없어 대회 목록/상세/갤러리 등 데이터 의존 화면은 이번 스윕에서 못 봤음 — 로컬
  Supabase 스택 띄운 뒤 재확인 필요. 새 CSS 프레임워크·새 브레이크포인트 체계·PWA는
  붙이지 않음(YAGNI).
