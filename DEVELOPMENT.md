...
- **'대회' 탭(대회 운영 페이지)에서는 팀 모집 마감 등 상태 메시지 숨김** — "대회" 탭은 대회
  목록에서 대회 이름을 클릭해 들어가는 화면, 즉 `ContestDetail.tsx` 전체(대회 운영 페이지)를
  가리킨다. 이 화면의 `팀 모집이 마감되었습니다 (현재 상태: …)` 같은 `lock-hint` 메시지
  (약 252번째 줄, 팀 만들기 폼 바로 아래)를 없앤다. 로그인한 사용자에게 항상 노출되던 것을
  제거하는 것이므로, 폼이 왜 안 보이는지 알려주던 역할을 다른 방식(예: 상태 스테퍼 아래 안내
  문구 재사용, 또는 완전히 제거)으로 대체할지 착수 시 확인.
