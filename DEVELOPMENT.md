...
- **아키텍처 트레이드오프 3건 대응 — 콜드 스타트 안내, 폴링 부하, GitHub 한도** (2026-09-07)
  아키텍처 드라이버 분석에서 드러난 리스크 3건을 각각 가장 싼 방법으로 막았다. 세 개 모두
  구조를 바꾸지 않고(모놀리스·REST 폴링 유지) 상한만 올리는 선택이다.
  - **콜드 스타트 — 안내로 이탈만 막고, 지연은 유료 플랜으로** (`App.tsx`, `style.css`)
    첫 대회 목록 요청이 2.5초 안에 안 오면 "서버를 깨우는 중입니다 N초" 안내를 띄운다
    (경과 초를 1초마다 갱신). **요청을 더 보내지는 않는다** — 이미 나간 그 요청이 곧 서버를
    깨우는 요청이라, 안내는 순수하게 체감용이다. 지연 자체를 없애는 건 Starter 플랜뿐이라
    위에 런북으로 정리했다. 외부 cron ping 은 무료 인스턴스 시간 상한·정책 문제로 채택하지
    않았다.
  - **스코어보드 서버 캐시 + 조건부 요청** (`views.py`, `settings.py`, `api.ts`)
    `scoreboard` 응답을 3초간 캐시한다(`SCOREBOARD_CACHE_SECONDS`, LocMemCache — Redis 를
    붙이지 않았다). 캐시 키는 **요청자 권한별로 분리**한다(`scoreboard:<slug>:judge|public`)
    — 하나로 합치면 캐시가 비공개 결선 순위를 관람객에게 그대로 돌려준다. 점수·팀·제출물이
    바뀌면 `invalidate_scoreboard()` 로 즉시 버리므로 **캐시 때문에 순위가 늦게 보이는 일은
    없다**(캐시는 갱신을 늦추는 게 아니라 같은 집계의 중복 계산만 없앤다).
    응답에는 직렬화 결과의 해시를 `ETag` 로 붙이고, 프론트는 다음 폴링에 `If-None-Match` 를
    실어 보낸다. 순위가 그대로면 서버가 `304`(본문 없음)를 주고 프론트는 **직전 배열을 같은
    객체 참조로** 돌려주므로 React 재렌더까지 건너뛴다. CORS 응답에서 `ETag` 를 읽으려면
    노출이 필요해 `CORS_EXPOSE_HEADERS = ['ETag']` 를 추가했다.
    **배포 후 실제 브라우저(Vercel 오리진)에서 폴링이 조용히 멈추는 버그를 발견해 별도로
    고쳤다** — django-cors-headers 의 기본 허용 헤더 목록에 `If-None-Match` 가 없어서,
    프리플라이트는 200으로 통과하는데 브라우저가 본 GET 요청 자체를 보내지 않았다. curl
    로는 재현되지 않고(프리플라이트를 안 거치므로) 실제 브라우저에서만 나타나는 종류의
    버그라 로컬에서 Vite + Django 를 함께 띄우고 폴링을 직접 지켜보다 잡았다. 서버 로그에는
    `OPTIONS` 만 남고 그 뒤로 `GET` 이 아예 없는 게 단서였다. `CORS_ALLOW_HEADERS =
    (*default_headers, 'if-none-match')` 로 고치고, 회귀 테스트 3건(프리플라이트 허용 헤더,
    ETag 노출, 304 응답도 `Access-Control-Allow-Origin` 을 유지하는지)을 추가했다 — 수정을
    되돌려서 테스트가 실제로 실패하는 것까지 확인했다. 총 77건 통과.
    접속자가 N명이면 5초마다 N번 돌던 집계가 3초에 1번으로 줄고, 대부분의 폴링이 본문 없는
    304 로 끝난다.
  - **폴링 지터** (`ContestDetail.tsx`) — `setInterval` 고정 주기를 매번 다시 예약하는
    `setTimeout` + ±20% 흔들림으로 바꿨다. 프로젝터·참가자·심사위원 화면이 한꺼번에 열려
    있어도 요청이 같은 초에 몰리지 않는다.
  - **GitHub 백엔드 프록시** (`contests/github.py`, `frontend/src/github.ts`)
    프론트가 `api.github.com` 을 직접 부르던 것을 `/api/github/<resource>/` 로 옮겼다.
    `GITHUB_TOKEN` 이 있으면 시간당 60회 → 5000회가 되고(토큰은 서버에만 있다), 응답을
    30분 캐시하므로 같은 저장소를 심사위원 여러 명이 열어도 GitHub 까지 나가는 건 한 번이다.
    **오픈 프록시가 되지 않도록** 받는 값은 저장소 URL 하나뿐이고 서버가 owner/repo 를 뽑아
    직접 API URL 을 조립한다(임의 URL 대리 요청 불가). 로그인 사용자 전용이며, 파일 경로에
    `..` 이 섞이면 거부한다. 트리에서 blob 만 남기고 500개 상한을 거는 것도 서버로 옮겨
    캐시·전송량을 줄였다. 프론트의 `GithubApiError` 종류(`not-found`/`rate-limit`/`error`)와
    화면 안내 문구는 그대로 유지된다.
  - 백엔드 테스트 14건 추가(스코어보드 캐시·304·권한별 캐시 분리·무효화 5건, GitHub 프록시
    익명 차단·비 GitHub URL 거부·캐시·토큰 미노출·경로 탈출 차단 등 9건), 총 74건 통과.
    프론트 `tsc -b && vite build` 통과.
    테스트 베이스 클래스 `ApiTestCase` 를 도입해 `_pre_setup` 에서 `cache.clear()` 한다 —
    LocMemCache 는 DB 와 달리 테스트마다 롤백되지 않아, 비우지 않으면 앞 테스트의 순위가
    뒤 테스트로 샌다.
...
- (선택) 커스텀 도메인 연결 — 비용·절차는 위 [커스텀 도메인 (예정)](#커스텀-도메인-예정-2026-09-03-조사) 참고.
  프론트만 먼저 붙이는 구성 추천.
- (선택) 대회 기간 Render Starter 플랜으로 슬립 방지 (월 $7) — 절차는
  [대회 당일 Starter 플랜 전환](#대회-당일-starter-플랜-전환-슬립-방지-런북) 참고
- (후속 과제) 스코어보드 WebSocket 전환 — 현재 5초 REST 폴링 + 서버 캐시 + 조건부 요청(304)
  으로 파일럿 규모(수십 팀·수 명의 심사위원)는 충분하고, 아래 최적화로 상한이 한 단계 더
  올라갔다. Render 단일 프로세스에서 Django Channels + Redis를 붙이는 비용 대비 이점이 작아
  계속 보류. 참가 팀이 수백 단위로 늘거나 폴링 부하가 문제 되면 재검토.
...
- **운영자가 대회를 삭제할 수 있는 기능 (테스트용 대회 정리용)** — 백엔드는 이미 가능한
  상태다: `ContestViewSet`(`ModelViewSet` + `IsOrganizerOrReadOnly`)에 `http_method_names`
  제한이 없어 운영자가 `DELETE /api/contests/<slug>/`를 호출하면 그대로 삭제되고, `Team`·
  `Award`가 `Contest`에 `CASCADE`로 걸려 있어 팀·참가자·제출물·심사위원·점수·시상까지 함께
  지워진다(연쇄 삭제 자체는 이미 마이그레이션에 존재). **없는 것은 프론트 UI뿐** — 삭제
  버튼이 어디에도 없다. 착수할 것: 대회 상세 화면(또는 목록)에 운영자 전용 삭제 버튼 추가,
  되돌릴 수 없고 딸린 데이터가 전부 함께 삭제된다는 확인 절차(예: 대회 이름을 다시 입력해야
  삭제되는 방식), 삭제 후 목록 화면으로 복귀. 백엔드 동작(운영자만 가능, 연쇄 삭제)을 굳히는
  회귀 테스트도 없으므로 함께 추가.
