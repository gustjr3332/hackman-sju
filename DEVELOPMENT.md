...
`backend/contests/tests.py`에 API 테스트 102건(인증·토큰 갱신, 대회 CRUD·상태 전환·삭제, 상태별
...
- **심사 보조: Claude API로 제출 저장소 사전 분석** (기획 확정, 미착수)
  ...
  **(5) 참가자에게는 보이지 않는다.** 결선 점수와 같은 등급으로 다룬다 — 운영자·배정된 심사위원만
  ...
  - 모델: `SubmissionReview`(`submission` OneToOne, `summary`, `findings` JSON, `cited_paths` JSON,
    `model`, `input_tokens`/`output_tokens`, `status`(pending/done/failed), `error`, `created_at`).
    제출물이 바뀌면 무효화(재분석 대상)로 표시.
  - 백엔드: `backend/contests/judge_assist.py` — 저장소 수집 → 프롬프트 구성 → Batch 제출 → 결과 저장.
    엔드포인트 `POST /api/contests/<slug>/analyze_submissions/`(운영자 전용, 일괄 제출),
    `GET /api/submissions/<id>/review/`(운영자·심사위원 전용).
  - 프론트: `SubmissionReview.tsx`(이미 GitHub 트리를 보여주는 심사 도구)에 분석 패널을 추가.
    대회 상세에 운영자용 "심사 보조 분석 실행" 버튼과 진행 상태 표시.
  - 설정: `ANTHROPIC_API_KEY`(필수, 없으면 기능 전체 비활성 — GitHub 토큰과 같은 방식),
    모델은 `claude-opus-5`. 의존성 `anthropic` 추가.

  **비용 감각:** 20팀 × 저장소 약 50k 입력 토큰 = 입력 1M 토큰. Opus 5 기준 입력 $5/1M 이므로
  대회 1회에 약 $5 + 출력분. Batch API 를 쓰면 절반. 캐시가 걸리면 재분석은 더 싸다.

  **착수 전 남은 확인:** `ANTHROPIC_API_KEY` 준비 여부, 심사 항목(예선=코드/기능, 결선=발표)에
  맞춰 분석 항목을 어떻게 나눌지, Batch 완료를 무엇으로 확인할지(운영자가 화면에서 새로고침 vs
  다음 폴링에 실어 보내기).

- **팀빌딩 보조: 참가자 프로필 + 팀 추천** (기획 확정, 미착수)

  **목표를 좁혀 잡는다: "혼자 온 사람이 30분 안에 팀을 찾게 한다".** 교내 해커톤 규모(참가자
  40~100명, 팀 10~20개)에서는 대부분 이미 아는 사람끼리 팀을 짠다. 전체 매칭을 최적화하려 들면
  만들 것만 커지고 효과는 안 보인다. 진짜 아픈 쪽은 그 안의 혼자 온 소수다.

  **지금 없는 것:** `User` 는 stock Django auth(username/email/password/is_staff)이고 `Participant`
  는 `(team, user)` 뿐이다. 기술 스택·경력·관심사 필드가 하나도 없다. 즉 이 기능의 대부분은 AI가
  아니라 **없는 프로필 데이터 레이어를 만드는 일**이다.

  ### 1단계 — 참가자 프로필 (LLM 이 실제로 값어치를 내는 곳)

  참가자는 체크박스 20개를 채우지 않는다. 자유 서술 한 문단을 받아 Claude 로 구조화한다:
  "웹 프론트 좀 했고 파이썬도 조금 압니다, 디자인도 관심 있어요" →
  `{프론트엔드: 중, Python: 하, 관심: [디자인, 백엔드], 성향: 학습지향}`.
  규칙으로는 못 하고 LLM 이 정확히 잘하는 일이다. 참가자당 1회, 짧은 입력·작은 JSON 출력.

  - 원문(자유 서술)과 추출 결과를 **둘 다 저장**한다. 추출이 틀렸을 때 참가자가 고칠 수 있어야
    하고, 추출 프롬프트를 바꾸면 원문에서 다시 뽑을 수 있어야 한다.
  - 추출 결과는 참가자가 확인·수정할 수 있게 보여준다(모델이 정한 것을 사실로 굳히지 않는다).
  - 1단계만 해도 그 자체로 쓸모가 있다 — 팀 목록에 "이 팀: 프론트 2, 백엔드 0"만 떠도 혼자 온
    사람이 어디로 갈지 알 수 있다. 2단계를 안 하더라도 여기서 끊을 수 있게 설계한다.

  ### 2단계 — 추천 (점수는 규칙, LLM 은 설명 한 줄만)

  사람 → 팀, 팀 → 사람 양방향 순위를 제시한다. **순위 계산은 규칙 기반 점수**로 한다(빈 역할
  채우기, 스택 겹침 최소화, 팀 인원 상한). 규칙 기반이어야 하는 이유:
  - 결정적이고, 왜 그 순서인지 설명이 되고, 공정성을 검증할 수 있다.
  - 팀 모집은 이제 모집중 상태에서만 열리고, 팀이 차는 동안 추천이 계속 바뀐다. 재계산이 잦은데
    매번 Claude 를 부르면 워커 1개짜리 서버(`backend/Procfile` 에 `-w` 없음)가 막힌다.
    **추출은 참가자당 1회, 매칭은 매번 규칙** — 이 분담이 구조적으로 맞다.

  LLM 은 상위 몇 개에 대해 "이 팀은 프론트가 비어 있고 당신 React 경험이 맞습니다" 정도의 한 줄
  설명만 붙인다(선택 기능 — 없어도 추천은 동작해야 한다).

  ### 3단계(자동 배정)는 만들지 않기로 확정

  모델이 팀을 배정하면 비결정적이고 공정성 검증이 안 되며, 무엇보다 **사회적으로 눈에 띈다** —
  "왜 나는 저 팀에 못 갔나"를 운영자가 답해야 한다. 심사 보조에서 점수를 대신 매기지 않기로 한
  것과 같은 이유다. **제안까지만 하고 선택은 사람이 한다.**

  ### 착수 전 남은 확인

  프로필을 대회별로 받을지 계정에 한 번만 붙일지(대회마다 관심사가 달라질 수 있음), 스택 태그를
  자유 문자열로 둘지 정해진 목록에 맞출지(집계·매칭은 목록이 편하고, 표현력은 자유 문자열이 낫다),
  모집중에 참가자가 한꺼번에 몰려 등록할 때 추출 호출을 어떻게 뒤로 미룰지(심사 보조와 같은
  워커 블로킹 문제 — Batch 또는 지연 처리).
