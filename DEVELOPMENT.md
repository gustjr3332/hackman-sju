...
`backend/contests/tests.py`에 API 테스트 122건(인증·토큰 갱신, 대회 CRUD·상태 전환·삭제, 상태별
...
정한 것을 사실로 굳히지 않는다). 자동 정리는 Anthropic·OpenAI·Google 중 키가 설정된 제공사만
선택지에 뜨고, 키가 하나도 없으면 그 버튼만 사라진다.
