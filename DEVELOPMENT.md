...
- **'대회' 탭에서는 팀 모집 마감 등 상태 메시지 숨김** — 현재
  `ContestDetail.tsx`의 `팀 모집이 마감되었습니다 (현재 상태: …)` 같은 `lock-hint` 메시지
  (약 252번째 줄, 팀 만들기 폼 바로 아래)가 로그인한 사용자 모두에게 항상 보인다. 요구사항은
  "'대회' 탭"에서는 이 메시지가 뜨지 않아야 한다는 것인데, 현재 화면에는 "대회"라는 이름의 탭이
  없다(스코어보드의 예선/결선 탭만 존재). 착수 전 사용자에게 "대회 탭"이 가리키는 화면 구획을
  확인해야 한다 — 대회 상세 화면 자체를 가리키는 것인지, 아니면 앞으로 만들 화면 구조(예: 대회
  정보/팀/심사 탭 분리)를 전제로 한 요청인지.
